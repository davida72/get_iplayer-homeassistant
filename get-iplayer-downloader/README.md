...
The addon comes with sensible defaults for downloading BBC Newsround. Here's the default configuration:
...
|--------|------|-------------|---------|
| `search_command` | string | Command to search for episodes | `get_iplayer --type=tv --channel=CBBC Newsround` |
| `download_command` | string | Command to download episodes | `get_iplayer --get {episode_index} --audio-only --output=/downloads --force --overwrite` |
...
- `{episode_index}` - Episode number to download
- `{input_file}` - Path to input file (ffmpeg)
- `{output_file}` - Path to output file (ffmpeg)

## Usage Examples

### Example 1: Download Audio from BBC Newsround

```yaml
search_command: "get_iplayer --type=tv --channel=CBBC Newsround"
download_command: "get_iplayer --get 1 --audio-only --output=/downloads --force --overwrite"
enable_ffmpeg: true
ffmpeg_command: "ffmpeg -i {input_file} -acodec mp3 -ab 128k {output_file}"
...
### Example 2: Download Full Video (No Conversion)

```yaml
search_command: "get_iplayer --type=tv 'Doctor Who'"
download_command: "get_iplayer --get 1 --output=/downloads --force --overwrite"
enable_ffmpeg: false
media_folder: "/media/tv_shows"
auto_delete_original: false
```

### Example 3: Download Radio Programme

```yaml
search_command: "get_iplayer --type=radio 'Today Programme'"
download_command: "get_iplayer --get 1 --type=radio --output=/downloads --force --overwrite"
enable_ffmpeg: true
